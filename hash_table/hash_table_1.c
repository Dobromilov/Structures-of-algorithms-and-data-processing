...
int insert(Node** table, int tableSize, int key, int* isNewKey) {
    ...
    *isNewKey = 1;  // Предполагаем, что ключ новый
    ...
    Node* current = head; //head=table[index]
    while (current != NULL) {
        if (current->key == key) {
    ...
    int collision = (head != NULL); // В ячейке 9 уже есть 'a' → head != NULL → collision = 1 
...
int main() {
    ...
    printf("Хеширование текста (1024 Б) методом прямого связывания \n");
    printf("+--------------------+---------------------+---------------------+\n");
